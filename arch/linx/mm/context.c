...
static int __init asids_init(void)
{
	...
	old = ssr_read(SSR_MMTBASE);
	asid_bits = old | (MMTBASE_ASID_MASK << MMTBASE_ASID_SHIFT);
	ssr_write(SSR_MMTBASE, asid_bits);
	asid_bits = (ssr_read(SSR_MMTBASE) >> MMTBASE_ASID_SHIFT)  & MMTBASE_ASID_MASK;
	asid_bits = fls_long(asid_bits);
	ssr_write(SSR_MMTBASE, old);
