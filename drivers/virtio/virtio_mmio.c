...
static void linx_virtio_mmio_debug_get(const char *stage, unsigned int offset,
				       unsigned int len)
{
	if (!linx_virtio_mmio_debug_enabled)
		return;

	linx_debug_uart_puts("LINX_VIRTIO_MMIO_GET stage=");
	linx_debug_uart_puts(stage);
	linx_debug_uart_puts(" offset=");
	linx_virtio_mmio_debug_hex(offset);
	linx_debug_uart_puts(" len=");
	linx_virtio_mmio_debug_hex(len);
	linx_debug_uart_puts("\n");
}

static noinline u8 linx_virtio_mmio_get_byte(void __iomem *base,
					    unsigned int offset)
{
	return readb(base + offset);
}

static void linx_virtio_mmio_get_bytes(void __iomem *base,
				      ...
				      unsigned int len)
{
	...
	switch (len) {
	case 1:
		ptr[0] = linx_virtio_mmio_get_byte(base, offset + 0);
		break;
	case 2:
		ptr[0] = linx_virtio_mmio_get_byte(base, offset + 0);
		ptr[1] = linx_virtio_mmio_get_byte(base, offset + 1);
		break;
	case 4:
		ptr[0] = linx_virtio_mmio_get_byte(base, offset + 0);
		ptr[1] = linx_virtio_mmio_get_byte(base, offset + 1);
		ptr[2] = linx_virtio_mmio_get_byte(base, offset + 2);
		ptr[3] = linx_virtio_mmio_get_byte(base, offset + 3);
		break;
	case 8:
		ptr[0] = linx_virtio_mmio_get_byte(base, offset + 0);
		ptr[1] = linx_virtio_mmio_get_byte(base, offset + 1);
		ptr[2] = linx_virtio_mmio_get_byte(base, offset + 2);
		ptr[3] = linx_virtio_mmio_get_byte(base, offset + 3);
		ptr[4] = linx_virtio_mmio_get_byte(base, offset + 4);
		ptr[5] = linx_virtio_mmio_get_byte(base, offset + 5);
		ptr[6] = linx_virtio_mmio_get_byte(base, offset + 6);
		ptr[7] = linx_virtio_mmio_get_byte(base, offset + 7);
		break;
	default:
		BUG();
	}
...
static inline void linx_virtio_mmio_debug_get(const char *stage,
					      unsigned int offset,
					      unsigned int len)
{
}
		   ...
		   void *buf, unsigned int len)
{
	...
	linx_virtio_mmio_debug_get("entry", offset, len);
...
#ifdef CONFIG_ARCH_LINX
	/*
	 * Linx bring-up currently has unstable 32-bit device-config reads in
	 * this path, and loop-generated byte reads have produced partial config
	 * copies. Read fixed-size virtio fields with explicit byte accesses
	 * until the underlying load/codegen issues are fixed. This only affects
	 * device-specific config space, not virtio-mmio control registers.
	 */
	linx_virtio_mmio_debug_get("modern-bytewise", offset, len);
	linx_virtio_mmio_get_bytes(base, offset, buf, len);
	return;
#endif
